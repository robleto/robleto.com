...
console.log("Notion Database ID 1:", process.env.NOTION_MUSICALS_DB_ID);
...
async function getMusicalsImages() {
	console.log("Fetching images from Notion...");
	const databaseId = process.env.NOTION_MUSICALS_DB_ID!;
...
async function downloadImages() {
	const publicPath = path.join(process.cwd(), "public", "lists/musicals");

	if (!fs.existsSync(publicPath)) {
		fs.mkdirSync(publicPath, { recursive: true });
		console.log("Created musicals directory.");
	}

	try {
		const images = await getMusicalsImages();
